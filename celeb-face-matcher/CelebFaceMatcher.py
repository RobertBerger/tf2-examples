...
# this does not work - use full tensorflow instead:
...
print('Runningsystem = ' + Runningsystem)
...
if Runningsystem == 'PC':
    ...
    embeddingpath = '/usr/share/celeb-face-matcher-models/'
...
if Loadtype == 'TL':
    try:
        ...
        interpreter = tf.lite.Interpreter(model_path)# This works if the
        # flite interpreter cannot be installed. This is the only TF version
    except ValueError as e:
        print(
            "Error: Modelfile could not be found. Check if you are in the correct workdirectory. Errormessage:  " +
            str(e))
...
dataE = [np.array(ImportedData['Embedding'][str(i)])
         for i in range(len(ImportedData['Name']))]
dataN = [np.array(ImportedData['Name'][str(i)])
         for i in range(len(ImportedData['Name']))]
dataF = [np.array(ImportedData['File'][str(i)])
         for i in range(len(ImportedData['Name']))]
...
# Choose version same as in " 2-Create embeddings database.py or jupyter"
def preprocess_input(x, data_format, version):
    x_temp = np.copy(x)
...
#-----------------------------------------------------------------------------
splitt = time()
celeb_embeddings = splitDataFrameIntoSmaller(
    dataE, int(np.ceil(len(dataE) / 4)))
...
if not cap.isOpened():
    ...
    cmd = "v4l2-ctl -d /dev/video0 --set-fmt-video=pixelformat=GRBG,width={width},height={height}".format(
        width=width, height=height)
    os.system(cmd)
    cmd = "v4l2-ctl -d /dev/video0 --set-selection=target=crop,left=0,top=4,width={width},height={height}".format(
        width=width, height=height)
    os.system(cmd)

    pipeline = "v4l2src device=/dev/video0 ! video/x-bayer,format=grbg,depth=8,width={width},height={height} ! bayer2rgb ! videoconvert !videoscale ! video/x-raw,width=640,height=400 !  appsink".format(
        width=width, height=height)
    if Runningsystem == 'PC':
...
# DECTECT FACE IN VIDEO CONTINUOUSLY
    faces_detected = face_cascade.detectMultiScale(
        frame, scaleFactor=1.2, minNeighbors=5)  # , Size(50,50))
    for (x, y, w, h) in faces_detected:
        rechteck = cv2.rectangle(
            frame, (x - p, y - p + 2), (x + w + p, y + h + p + 2), (0, 255, 0), 2)
...
# DETECT KEY INPUT  - ESC OR FIND MOST CENTERED FACE
    ...
    if key == 32:
        ...
        mitte = ()
        if len(
                faces_detected) != 0:  # only if the cascader detected a face, otherwise error
            start1 = time()
...
# FRAME THE DETECTED FACE
            ...
            # use only the detected face; crop it +2 to remove frame # CHECK IF
            # IMAGE EMPTY (OUT OF IMAGE = EMPTY)
            img = frame[y - p + 2:y + h + p - 2, x - p + 2:x + w + p - 2]

            if len(
                    img) != 0:  # Check if face is out of the frame, then img=[], throwing error
...
# IMAGE PREPROCESSING
                ...
                if inputtype == 'int':
                    ...
                    # data_format= None, 'channels_last', 'channels_first' . If
                    # None, it is determined automatically from the backend
                    samples = preprocess_input(
                        samples, data_format=None, version=3).astype('int8')
                else:
                    ...
                    # None, it is determined automatically from the backend
                    samples = preprocess_input(
                        samples, data_format=None, version=2)
                # now using the tflight model
                print('preprocess data for model', time() - start4)
# CREATE FACE EMBEDDINGS
                if Loadtype == 'armNN':
                    prep = time()
                    input_tensors = ann.make_input_tensors(
                        [input_binding_info], [samples])
                    # Get output binding information for an output layer by
                    ...
                    start42 = time()
                    EMBEDDINGS = ann.workload_tensors_to_ndarray(
                        output_tensors)
                elif Loadtype == 'TL':
                    prep = time()
                    ...
                    print('ANN preperation ', time() - prep)
                    start42 = time()
                    EMBEDDINGS = interpreter.get_tensor(
...
# READ CELEB EMBEDDINGS AND COMPARE
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                    ergebniss_1 = executor.submit(
                        faceembeddingNP, EMBEDDINGS, np.array(
                            celeb_embeddings[0]))
                    ergebniss_2 = executor.submit(
                        faceembeddingNP, EMBEDDINGS, np.array(
                            celeb_embeddings[1]))
                    ergebniss_3 = executor.submit(
                        faceembeddingNP, EMBEDDINGS, np.array(
                            celeb_embeddings[2]))
                    ergebniss_4 = executor.submit(
                        faceembeddingNP, EMBEDDINGS, np.array(
                            celeb_embeddings[3]))

                if ergebniss_1.done() & ergebniss_2.done(
                ) & ergebniss_3.done() & ergebniss_4.done():
...
# PLOT IMAGES
                ...
                if np.shape(Beleb) != (width, height):
                    Beleb = cv2.resize(
                        Beleb,
                        (np.shape(img_small)[0],
                         np.shape(img_small)[1]),
                        interpolation=cv2.INTER_AREA)

                if largeImg:
                    larg = time()
                    img_small2 = cv2.resize(
                        img_small, ImgSize, interpolation=cv2.INTER_LINEAR)
                    Beleb2 = cv2.resize(
                        Beleb,
                        (np.shape(img_small2)[0],
                         np.shape(img_small2)[1]),
                ...
                Text = str(dataN[idx[0].round(2)]) + \
                    ' EuDist: ' + str(EuDist[idx[0]])
                ...
                print('-------------------------------------')
                print('Distance value: ', EuDist[idx[0]].round(2), ' | ', 'Name: ', str(
                    dataN[idx[0]]), ' | ', ' Filename: ', str(dataF[idx[0]]))
